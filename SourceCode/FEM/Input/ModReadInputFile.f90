...
!!------------------------------------------------------------------------------------------------
! Modifications:
! Date: 2022/09 (Osmotic Pressure Analysis)         Authors: Rafael  Geronimo 
!                                                            Jose Luis Thiesen
!                                                            Bruno Klahr 
!##################################################################################################
module ModReadInputFile
    ...
    integer,parameter :: iAnalysisSettings=1, iLinearSolver=2, iNonLinearSolver=3,iLineSearchAlgorithm=4, iStaggeredSplittingScheme=5, iMaterial=6, &
                         iPermeability=7, iOsmoticPressure=8, iMeshAndBC=9, iMacroscopicDispAndDeformationGradient=10, iMacroscopicPressureAndGradient=11, &
                         nblocks=11
    ...
    contains
        ...
        subroutine ReadInputFile( FileName, AnalysisSettings , GlobalNodesList , ElementList , BC , BCFluid , NLSolver )
            ...
            type(ClassOsmoticPressureModelWrapper) , pointer , dimension(:) :: OsmoticPressureList
            ...
            BlockName(8)="Osmotic Pressure"
            BlockName(9)="Mesh and Boundary Conditions"
            BlockName(10)="Macroscopic Displacement And Deformation Gradient"
            BlockName(11)="Macroscopic Pressure And Gradient"
        ...
        !---------------------------------------------------------------------------------------------------------------------------------------------------------
         ...
         !---------------------------------------------------------------------------------------------------------------------------------------------------------
                    case (iOsmoticPressure)
                        if (.not.BlockFound(iPermeability)) call Datafile%RaiseError("Permeability must be specified before the Osmotic Pressure.")
                        call ReadOsmoticPressureModel(AnalysisSettings,OsmoticPressureList, MaterialList, DataFile)

        !---------------------------------------------------------------------------------------------------------------------------------------------------------
                    case (iMeshAndBC)
                        if (.not.all(BlockFound([iOsmoticPressure]))) call DataFile%RaiseError("Osmotic Pressure must be specified before mesh.")
                        call ReadMeshAndBC(DataFile,GlobalNodesList,ElementList,AnalysisSettings,MaterialList,PermeabilityList,OsmoticPressureList, BC,BCFluid,TimeFileName)
        ...
        subroutine ReadAnalysisSettings(DataFile,AnalysisSettings)
            ...
            ListOfOptions=["Problem Type","Analysis Type","Nonlinear Analysis","Hypothesis of Analysis", &
                            ...
                            "Multiscale Epsilon Parameter","Multiscale Model Fiber Axial Direction","Osmotic Pressure Analysis"]
            ...
            ! Option Osmotic Pressure Analysis
            if (DataFile%CompareStrings(ListOfValues(15),"True")) then
                AnalysisSettings%OsmoticPressureAnalysis= .true.
                AnalysisSettings%OsmoticPressureActivator = 1.0d0
            elseif (DataFile%CompareStrings(ListOfValues(15),"False")) then
                AnalysisSettings%OsmoticPressureAnalysis= .false.
                AnalysisSettings%OsmoticPressureActivator = 0.0d0
            else
                call Error( "Osmotic Pressure Analysis not identified" )
            endif
        ...
        !=======================================================================================================================
        subroutine ReadMeshAndBC(DataFile,GlobalNodesList,ElementList,AnalysisSettings,MaterialList,PermeabilityList,OsmoticPressureList,BC,BCFluid,TimeFileName)
            ...
            select case (PreProcessorID)
                ...
                case (PreProcessors%HyperMesh)
                    ...
                    call ReadMeshHyperMesh(FileNumber,GlobalNodesList,ElementList,AnalysisSettings,MaterialList,PermeabilityList, OsmoticPressureList, PreProcessorID)
        ...
        !=======================================================================================================================
        subroutine ReadOsmoticPressureModel(AnalysisSettings,OsmoticPressureList, MaterialList, DataFile)
            implicit none
            ...
            integer :: NumberOfMaterials,  i, MaterialID, ModelEnumerator
            integer :: OsmoticPressure_block_cont_lines
            character(len=100):: ModelName, string, OptionName, OptionValue
            ...
            if (AnalysisSettings%ProblemType .eq. ProblemTypes%Biphasic) then
                ...
                ! Check if number of materials is equal in Material and permeability
                if( NumberOfMaterials .ne. size(MaterialList)) then
                    call DataFile%RaiseError("Expected the same Number of Materials in Material and Osmotic Pressure blocks in Settings.dat") 
                endif
              
                call DataFile%CheckError
            
                allocate(OsmoticPressureList(NumberOfMaterials))
                ...
                do i=1,NumberOfMaterials
                    ...
                    OsmoticPressureList(i)%MaterialID = MaterialID
        
                    call DataFile%GetNextString(ModelName)
                    call DataFile%CheckError
        
                    call OsmoticPressureModelIdentifier(ModelName, AnalysisSettings, ModelEnumerator)
        
                    OsmoticPressureList(i)%ModelEnumerator = ModelEnumerator
        
                    call AllocateOsmoticPressureModel(ModelEnumerator, AnalysisSettings, 1, OsmoticPressureList(i)%Mat)
        
                    call OsmoticPressureList(i)%Mat(1)%ReadOsmoticPressureParameters(DataFile)
        
                enddo
            endif

            OsmoticPressure_block_cont_lines = 0
            do while(.not. BlockFound(iOsmoticPressure)) 
                OsmoticPressure_block_cont_lines = OsmoticPressure_block_cont_lines + 1
                call DataFile%GetNextString(string)
                if (DataFile%CompareStrings(string,'end'//trim(BlockName(iOsmoticPressure)))) then
                    BlockFound(iOsmoticPressure)=.true.
                elseif(OsmoticPressure_block_cont_lines>200) then
                    call DataFile%RaiseError("End of block was expected. BlockName="//trim(BlockName(iOsmoticPressure)))
                endif
            enddo
        ...
        !=======================================================================================================================
        subroutine ReadMeshHyperMesh(FileNumber, GlobalNodesList, ElementList, AnalysisSettings, MaterialList, PermeabilityList,OsmoticPressureList, PreProcessorID)
            ...
            do i = 1,size(ElementMaterialID)
                ...
                do j=1,size(MaterialList)
                    if (MaterialList(j)%MaterialID == ElementMaterialID(i) .and. AnalysisSettings%ProblemType .eq. ProblemTypes%Biphasic) then
                        ...
                        !if (AnalysisSettings%OsmoticPressureAnalysis == .true.) then
                        !    OsmoticPressure => OsmoticPressureList(j)
                        !endif
                ...
                if (AnalysisSettings%ProblemType .eq. ProblemTypes%Biphasic) then
                    ...
                    !if (AnalysisSettings%OsmoticPressureAnalysis == .true.) then
                    !    call OsmoticPressureConstructor(  ElementBiphasic, ElementList, GlobalNodesList, Permeability, OsmoticPressure, AnalysisSettings )
                    !endif 
