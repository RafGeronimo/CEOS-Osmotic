...
module ModElementBiphasic
    ...
    type, extends (ClassElement) :: ClassElementBiphasic
        ...
        class(ClassOsmoticPressureModel), pointer, dimension(:) :: GaussPoints_Osmotic => null()
    ...
    contains
            ...
            procedure :: AllocateGaussPoints_Osmotic
    ...
    contains
        ...
        !==========================================================================================
        subroutine AllocateGaussPoints_Osmotic(this,nGP)
            implicit none
            class(ClassElementBiphasic) :: this
            integer , intent(inout) :: nGP
            stop "Erro::AllocateGaussPoints_Osmotic::Dummy"
        end subroutine
        !==========================================================================================
        ...
        subroutine ElementStiffnessMatrix_Kuu( this, Pe, Ke, AnalysisSettings )
            ...
            real(8)                             :: Lambda, DeltaPi 
            integer                             :: UndrainedActivator
            real(8)                             :: OsmoticPressureActivator
            ...
            do gp = 1, size(NaturalCoord,dim=1)
                ...
                this%GaussPoints_Osmotic(gp)%J = J_currentStaggered
                
                call this%GaussPoints_Osmotic(gp)%GetFixedChargeDensityAndOsmoticGradient(AnalysisSettings%LC, AnalysisSettings%ST)
                
                call this%GaussPoints_Osmotic(gp)%GetLambda(AnalysisSettings%LC, AnalysisSettings%ST)
                
                
                P_CurrentStaggered = P_CurrentStaggered + (this%GaussPoints_Osmotic(gp)%DeltaPi*AnalysisSettings%OsmoticPressureActivator)
                ...
                !Calculate the Osmotic Pressure term for the tangent matrix:
                call MatrixMatrixMultiply_TransB(bs, bs, Ke, -this%GaussPoints_Osmotic(gp)%Lambda*AnalysisSettings%OsmoticPressureActivator*Weight(gp)*detJ*FactorAxi, 1.0d0) !C := alpha*(A)*B^T + beta*C
        ...
        subroutine ElementInternalForce_solid(this,AnalysisSettings, Pe, Fe, Status)
            ...
            do gp = 1, size(NaturalCoord,dim=1)
                ...
                Fe = Fe - hs*(P_CurrentStaggered*FactorAxi*Weight(gp)*detJ)
